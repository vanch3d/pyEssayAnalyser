...
from EssayAnalyser.se_stops import essay_stop_words
    ...
    while 1:
        if p_counter <= len(text) - 1:
            ...
            if (re.search('^[0-9]+', para[
                0]) and numsq == 'nonums'):  # If the first character of the paragraph is a number, this is probably not a reference but a footnote and should not be counted as a reference. See A1862829-H810-10I_02-1-U.
                p_counter += 1
            ...
            ##                print('\n~~~~~~~~~~~~get_essay_body 1', para)
                                ...
                                para)):  # If there are no alphabetic characters in this paragraph, this is not a reference. See X8204920-H810-10I_02-1-U.txt.
                p_counter += 1
...
def get_essay_body(parasenttok, nf, dev):
    ...
    if responseR == True:
        ...
        position, appendix_head_index, responseA = find_appendix_head(parasenttok, refs_head_index, nf,
                                                                      dev)  # See if there is an appendix following the refs section
        if appendix_head_index != [] and position == 'after':  # If the appendix occurs after the refs...
            ...
            refs_counter = count_refs(parasenttok[x:y],
                                      'nonums')  # Count refs from the refs heading to the beginning of the appendices
            ...
            x = refs_head_index + 1
            refs_counter = count_refs(parasenttok[x:],
                                      'nonums')  # Count refs from the refs heading to the end of the text
            if refs_counter == 0 and x != parasenttok[
                -1]:  # If no refs have been found despite there being a refs heading and there is text beyond the refs heading
                # print('\n\n\n\n\n******************refs_counter 1', refs_counter)
                refs_counter = count_refs(parasenttok[x:],
                                          'nums')  # This may be because the author has put numbers in front of the references, in which case, try counting the refs without dismissing number-initial lines.
        ...
        wc_index, newtext, responseW = find_word_count_index(parasenttok, nf,
                                                             dev)  # Look for string 'word count' (any case, search from end backwards)
        if responseW == True:
            if dev == 'DGF':
                print('~~~~~~~~~~~~get_essay_body found a word count phrase~~~~~~~~~~~')
            last_body_para_index = wc_index
            x = wc_index + 1
            position, appendix_head_index, responseA = find_appendix_head(parasenttok, wc_index, nf,
                                                                          dev)  # See if there is an appendix following the refs section
            if appendix_head_index != []:
                y = appendix_head_index - 1
                refs_counter = count_refs(parasenttok[x:y],
                                          'nonums')  # Count refs from the word count to the appendix heading
                # print('\n\n~~~~~~~~~~~~APPENDIX SECTION~~~~~~~~:', responseA, parasenttok[appendix_head_index:])
            else:
                refs_counter = count_refs(parasenttok[x:],
                                          'nonums')  # Count refs from the word count to the end of the text
                if refs_counter == 0 and x != parasenttok[
                    -1]:  # If no refs have been found despite there being a word count and there is text beyond the word count heading
                    print('\n\n\n\n\n******************refs_counter 2', refs_counter)
                    refs_counter = count_refs(parasenttok[x:],
                                              'nums')  # This may be because the author has put numbers in front of the references, in which case, try counting the refs without dismissing number-initial lines.
...
def find_appendix_head(parasenttok, refs_head_index, nf, dev):
    ...
    while 1:
        if p_counter <= len(parasenttok) - 1:  # ... and work forwards
            ...
            firstsent = parasenttok[p_counter][
                0]  # Get the first sentence only of the paragraph. Note it is not word tokenised.
            if len(firstsent) < 70:  # xxxx watch this
                if (
                        'Appendix' in firstsent or 'APPENDIX' in firstsent or 'Appendices' in firstsent or 'Annex' in firstsent):
                    if dev == 'DGF':
                        result = True
                        print('~~~~~~~~~~~~~find_appendix_head succeeds 1 with p_counter =: ', p_counter,
                              parasenttok[p_counter])
    ...
    while 1:  # Start at refs heading and work backwards
        if p_counter >= 0:  # ... and stop when you reach the beginning of the text
            ...
            firstsent = parasenttok[p_counter][
                0]  # Get the first sentence only of the paragraph. Note it is not word tokenised.
            if len(
                    firstsent) < 50:  # Note I'm not sure what this number should be. Needs to be lower than 54 for this essay: Y432872X-H810-12I_02-1-M.txt
...
def find_word_count_index(parasenttok, nf, dev):
    ...
    while 1:
        if p_counter >= 0:
            para = ' '.join(parasenttok[p_counter])
            if re.search(p, para) and p_counter > len(
                    parasenttok) / 2:  # If phrase 'word count' occurs in this para, and this para is in the second half of the essay (avoids matching cases in the front pages)
                # if dev == 'DGF':
                # print('\n\n~~~~~~~~~~~~~find_wc_index SUCCEEDS~~~~~~~~~~~')
                a = p_counter + 1
                newtext = parasenttok[:a]
                return p_counter, newtext, True
            elif re.search(q, para) and p_counter > len(
                    parasenttok) / 2:  # If phrase 'xxxx words' occurs in this para, and this para is in the second half of the essay (avoids matching cases in the front pages)
                # if dev == 'DGF':
                # print('\n\n~~~~~~~~~~~~~find_wc_index SUCCEEDS~~~~~~~~~~~')
                a = p_counter + 1
                newtext = parasenttok[:a]
    ...
    while 1:
        if p_counter >= 0:
            ...
            firstsent = parasenttok[p_counter][
                0]  # Get the first sentence only of the paragraph. Note it is not word tokenised.
            if (re.search(r'(^|\n)[0-9]+',
                          para)  # para starts with one or more numbers (For TMA01_H810_V_1.0_T5218396_utf8)
                ...
                return p_counter, newtext, True
                    ...
                    firstsent) < 45:  # xxxx This doesn't work even for tma01: and p_counter > len(parasenttok)/2: # The refs heading must be in the second half of paragraphs. This may not work for TMA02.
...
def edit_text(text):
    ...
    text = re.sub(r'Ph\.\s*D\.', "PhD ",
                  text)  # 'Ph. D.' with 'PhD ' # Note you need to do this and all changes including spaces before text is split on whitespace.
    text = re.sub(r'Ph\.\s*D', "PhD ",
                  text)  # 'Ph. D' with 'PhD ' # Note you need to do this and all changes including spaces before text is split on whitespace.
    text = re.sub(r'et\s+al\.', "et al", text)  # 'et al.' with 'et al' (varying whitespace)
    text = re.sub(r'et\.\s+al\.', "et al",
                  text)  # 'et. al.' with 'et al' (varying whitespace) # not correct but nevertheless used A2002667-H810-10I_02-1-U
    ...
    text = [re.sub(r'(^|\n)([a-zA-Z]+[hH])-([a-zA-Z]+)', r'\1\2hhhhhh\3', w) for w in
            text]  # 'graph-based' with 'graphhhhhhhbased'
    text = [re.sub(r'(^|\n)([a-zA-Z]+)-([a-zA-Z]+)-([a-zA-Z]+)', r'\1\2hhhhh\3hhhhh\4', w) for w in
            text]  # 'face-to-face' with 'facehhhhhtohhhhhface'
...
def reinstate_hyphens1(sent):
    ...
    temp = [re.sub(r'([a-zA-Z][hH]+)hhhhhh([a-zA-Z]+)', r'\1-\2', w) for w in
            sent]  # 'graphhhhhhhbased' (any case) with 'graph-based'
    temp = [re.sub(r'([a-zA-Z]+)hhhhhh([a-zA-Z]+)', r'\1-h\2', w) for w in
            temp]  # 'stakehhhhhholder' with 'stake-holder'
    temp = [re.sub(r'([a-zA-Z]+)hhhhh([a-zA-Z]+)hhhhh([a-zA-Z]+)', r'\1-\2-\3', w) for w in
            temp]  # 'facehhhhhtohhhhhface' with 'face-to-face' with
...
def remove_punc_fm_sents(sent):
    ...
    # print(sent)
    temp = [w for w in sent if not re.match('[\.\|\?\+\(\)\{\}\[\]\^\$\\\'\"`!,;/\=%\*@\&\<\>\[\]\{\}~#]+$',
                                            w)]  # This gets rid of tokens that constitute a series of ASCII punctuation and symbols and that do not end in symbols omitted from the list, e.g., hyphen and colon-
    # print(temp)
    myfunnychars = [u'\u2019', u'\u2019\u002C', u'\u2019\u002E', u'\u2018', u'\u201a', u'\u201b', u'\u201c',
                    # A crib for these symbols is in file unicode_symbol_codes.xlsx
                    u'\u201d', u'\u201e', u'\u201f', u'\ufeff',
                    u'\u2026', u'\u27a2', u'\ufeff', u'\xb9', u'\xb2',
                    u'\xb3', u'\u2013', u'\u2014', u'\u20AC', u'\xa3', u'\x5c', u'\xa8',
                    # u'\xb7',  # this 'middle dot' is used as a list enumerator by B4375120-H810-10I_01-1-U_utf8.txt
                    ...
                    u'\u2212', u'\u21B5\x2a' u'\xB0', u'\xB1', u'\xD7', u'\u03BB', u'\xA9', u'\u03BE', u'\u03C6',
                    u'\u03B1',  # xxxx From this line onwards were put in for the abstract analyses.
    ...
    temp = [w for w in temp if not re.match(u'\u2026', w[
        0])]  # This finds sequences of this character, needed for A4672317-H810-11I_01-1-U_utf8
    # Be careful not to remove the bullet points and hyphens from the text, just test to see if they are there.
    temp2 = [w for w in temp if not re.match(u'\u2022', w[
        0])]  # This finds one or more bullet points. Note that bullet points are not being removed at this point. Nor are hyphens.
    temp2 = [w for w in temp2 if not re.match('-', w[0])]  # This finds one or more hyphens
    if (
            temp2 == []):  # If removing all the punc marks leaves an empty sentence      # or re.match(\[[a-zA-Z]\] ,temp): # or a single letter
        result = ['#-s:p#'] + temp  # label this sentence as punctuation                
    else:
        result = [
                     '#dummy#'] + temp  # Otherwise put a 'dummy' label in as a place-holder for proper labels to be added later.
    return result


def remove_punc_fm_sents2(sent):
    sent = [w for w in sent if not re.match(r'^o$', w)]  # Gets rid of single letter 'o' (used as a bullet point).
    sent = [w for w in sent if
            not re.match(r'^[0-9]+$', w)]  # Gets rid of single letter 'o' (used as a bullet point) and integers.
    ...
    sent = [w for w in sent if not re.match(u'-',
                                            w)]  # Gets rid of a token that is a hyphen or a series of hyphens (not hyphens in the middle of hyphenated words). These often used like bullet points. Should not interfere with hyphenated words.
...
def find_and_label_numeric_sents(sent):
    if sent[0] == '#-s:p#':  # Just pass on  a sent that has already been labelled as punc
        return sent
    enums = ['i', 'ii', 'iii', 'iv', 'v', 'vi', 'vii', 'viii', 'ix', 'x', 'xi',
             'xii']  # xxxx testing this. It might create odd results.
    ...
    temp2 = [item for item in temp1 if not re.match('[\.\|\*\?\+\(\)\{\}\[\]\^\$\\\'!,;:/-]+',
                                                    item)]  # Get every item in the sent that doesn't contain punc: [u'4', u'-', u'9'] from B8040371-H810-11I_02-1-U
    if temp2 == [
        '#dummy#']:  # If the sentence is empty (apart from a 'dummy' label) after all tokens that contain numbers and list itemisers have been removed
...
def count_words(sent):
    # print(sent)
    mylist = []
    mylabels = ['#+s#', '#+s:i#', '#+s:c#', '#+s:s#', '#+s:p#', '#-s:e#',
                '#+s:b#']  # xxxx 'dummy' is the label of things that have not been identified. Much more likely not to be prose than to be prose, so I've left it out.
    if sent[
        0] in mylabels:  # Only count the words of the true sentences (prose), table entries and list items. These are the same items that are considered for the key word graph.
...
def get_lemmas(sent):
    ...
    # print(sent1)
    wordnet_tag = {'NN': 'n', 'NNS': 'n', 'NNP': 'n', 'NNPS': 'n', 'JJ': 'a', 'JJR': 'a', 'JJS': 'a', 'VB': 'v',
                   'VBD': 'v', 'VBG': 'v', 'VBN': 'v', 'VBP': 'v', 'VBZ': 'v', 'RB': 'r', 'RBR': 'r', 'RBS': 'r'}
    for t in sent1:
        try:
