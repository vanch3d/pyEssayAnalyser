...
def getAssignmentData(module, assignment):
    ...
    ass_q_long_le = os.path.join(cwdir, '..' + os.sep + 'data' + os.sep,
                                 module + '_' + assignment + '_ass_q_long_le.txt')
    f = pickle.load(open(ass_q_long_le, 'rb'))
    ass_q_long_lemmd = f

    ass_q_long_le2 = os.path.join(cwdir, '..' + os.sep + 'data' + os.sep,
                                  module + '_' + assignment + '_ass_q_long_le2.txt')
    f = pickle.load(open(ass_q_long_le2, 'rb'))
    ass_q_long_lemmd2 = f

    ass_q_short_le = os.path.join(cwdir, '..' + os.sep + 'data' + os.sep,
                                  module + '_' + assignment + '_ass_q_short_le.txt')
...
def top_level_procedure(essay_txt, essay_fname, nf, nf2, dev, module, assignment):
    ...
    gr_se_sample = sample_nodes_for_figure(gr_se, truesents,
                                           'se')  # Get a sample of the nodes from the sentence graph to make a figure with later
    # print '\nSE sample graph'
    # print(gr_se_sample.adj)    # This is how you print a networkx graph

    # print '\nKE sample graph start'
    gr_ke_sample = sample_nodes_for_figure(gr_ke, keylemmas,
                                           'ke')  # Get a sample of the nodes from the sentence graph to make a figure with later
    # print '\nKE sample graph'
    ...
    if dev == 'DGF':
        ...
        debora_write_results_se(essay_fname, \
                                ...
                                introheaded, i_first, i_last, i_toprank, countIntroSent, countIntroChars,
                                percent_body_i, \
    ...
    myheadings = ['#-s:H#', '#-s:s#', '#-s:d#', '#-s:l#', '#-s:c#', '#-s:q#', '#-s:e#',
                  '#-s:b#']  # Map these back to old heading label
