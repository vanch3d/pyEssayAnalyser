    ...
    while 1:
        ...
        if len(window_words) == 2:
            try:  # 'add_edge' is defined in 'pygraph\classes\digraph'
                gr_ke.add_edges_from([(window_words[0][1], window_words[1][
                    1])])  # add the edges between the key lemmas not the inflected real words
...
def keywords2ngrams(keywords, text, n):
    ...
    for para in text:
        for sent in para:
            ngrams, newsent = keywords2ngrams_i(keywords, sent,
                                                n)  # make n-grams out of sequences of key words that occur within-sentence
            if ngrams != []:
                ...
                para.insert(i,
                            newsent)  # Insert the new sentence (with the ngram phrase deleted) where you deleted the old one. Note that this update the text variable.
    mylist = [x for y in mylist for x in
              y]  # Unnest - get rid of the paragraph nesting to make a straight list of n grams. Don't use 'flatten', because you still need lists of ngrams.
    tuples = []
    for ngram in mylist:
        y = sum(1 for x in mylist if
                x == ngram)  # Count the occurrences of this ngram in the full list of ngrams == occurrences in text
        tuples.append((ngram,
                       y))  # Put each ngram in a tuple with its number of occurrences. # [(['course', 'production'], 1), (['disabled', 'students'], 4),
    mylist3 = []
    for item in tuples:
...
def keywords2ngrams_i(keywords, sent, n):
    ...
    while 1:
        ...
        if len(win_words) == n:
            if n == 4:
                if win_words[0] in keywords and win_words[1] in keywords and win_words[2] in keywords and win_words[
                    3] in keywords:
...
def cf_ngrams_section(keywords, ngrams, text_se, label):
    ...
    if label == '#+s:i#':
        temp = [item for item in temp if item[0][
            0] == '#+s:i#']  # [('#+s:i#', 'NN'), ('report', 'report'), ('outlines', 'outline'), ('main', 'main'),
    elif label == '#+s:c#':
        temp = [item for item in temp if item[0][
            0] == '#+s:c#']  # [('#+s:i#', 'NN'), ('report', 'report'), ('outlines', 'outline'), ('main', 'main'),
    else:
        temp = temp
    if ngrams != []:
    ...
    while 1:
        if counterB <= len(text) - 1 and n == 2:  # xxxx Note that this is set up only to deal with bigrams.
            temp = [text[counterA][0], text[counterB][
                0]]  # xxxx If I want to do trigrams or other n-grams, I need to change this to deal with longer phrases.
    ...
    count1 = sum(
        1 for item in mylist)  # The number of distinct key bigrams found in the assignment question or intro or concl
...
def process_essay_ke(text, wordtok_text, nf, nf2, dev):
    ...
    for item in unique_lemma_set:  # item: fox
        temp = [w[0] for w in text if
                item == w[1]]  # temp: ['foxes', 'foxes', 'foxes', 'fox', 'foxes', 'foxes', 'foxes', 'fox']
...
def debora_write_results_ke(text_ke, text_se, gr_ke, di, edges_over_sents, myarray_ke, threshold_ke, \
                            keylemmas, keywords, fivemostfreq, bigram_keyphrases, trigram_keyphrases,
                            quadgram_keyphrases, \
    ...
    nf2.write('; ')
    nf2.write(
        'ke centr thrshld; ')  # (20,.03) # Centrality score threshold_ke expressed as '(percentage_of_true_sents, centrality_score)' tuple defining what qualifies as a key lemma
    ...
    nf2.write('; ')
    x = sum(item[1] for item in
            bigram_keyphrases)  # the total number of bigrams made up from key words including repetitions of bigrams.
    ...
    nf2.write('; ')
    nf2.write(
        'bigrams in ass_q; ')  # 'Total number of key bigrams in the essay that also occur in the assignment question
    ...
    nf2.write('; ')
    nf2.write(
        'sum_freq_kls_in_tb_index; ')  # Sum of frequencies of the essay's key lemmas that occur in the text book index
...
def get_essay_stats_ke(text_se, gr_ke, di, myarray_ke, keylemmas, keywords, \
    ...
    while 1:
        if counter <= len(di) - 1:
            lemma = di[counter][0]
            freqlemma = len(myarray_ke[
                                lemma])  # The frequency of the inflected forms of this lemma in the essay is the length of the list of inflected forms associated with this lemma in the array
    ...
    all_bigrams = sum(item[1] for item in
                      bigram_keyphrases)  # the total number of bigrams made up from key words including repetitions of bigrams.
    if len(scoresNfreqs) > 0:  # edge case condition
        x = scoresNfreqs[0][1]
        topbetscore = round(x, 3)
