...
def add_to_sentence_array(myarray, text):
    ...
    for para in text:
        for sent in para:
            add_item_to_array(myarray, counter,
                              sent)  # Add the counter'th sentence to the array with counter as its key
            counter += 1
...
def find_cosine_similarity(counter1, counter0, myWarray, myCarray, dev):
    ...
    for w in myWarray[counter0]:  # Now repeat the process for sent0, but...
        if w not in myWarray[
            counter1]:  # make sure this word has not already been dealt with above in processing for sent1...
            listposition_y = myWarray[counter0].index(w)  # Get the list position of w in sent0 from the Word array
            score_y = myCarray[counter0][
                listposition_y]  # Get the score for w in sent0 from the Count array
...
def add_one_nodes_edges(gr, counter1, counter0, myWarray, myCarray, dev):
    ...
    while 1:
        if counter1 <= (len(
                myWarray) - 1) and counter1 != counter0:  # Stop looping when counter1 (instantiated earlier) reaches the total number of sentences and make sure 'from' and 'to' nodes are not the same node.
            weight = float(find_cosine_similarity(counter1, counter0, myWarray, myCarray, dev))
            if weight > 0:
                gr.add_weighted_edges_from([(counter1, counter0,
                                             weight)])  # xxxx @directedness - both 'directed backards' and 'undirected'. Add the current edge with the weight you have calculated. This line adds the backwards-directed edges for the current node (counter0).
...
def add_all_node_edges(gr, myWarray, myCarray, nf2, dev):
    ...
    while 1:
        if counter0 <= (len(
                myWarray) - 1) and counter1 > counter0:  # Stop looping when the counter reaches the total number of sentences
            ...
            zeroweights = add_one_nodes_edges(gr, counter1, counter0, myWarray, myCarray,
                                              dev)  # Add all the edges for one node (the 'to' node)
...
def find_global_weight_score(Vi, gr, d, min_value, graph_size, scores_array, i):
    ...
    list0 = gr.predecessors(
        Vi)  # xxxx @directedness 'directed'. Do not delete. Find out which nodes point to Vi, i.e., ALL its predecessors. Directed graph.
    ...
    if list0 == []:  # If Vi has no predecessors
        ...
        for Vj in list0:  # For each node Vj that points to Vi
            ...
            WoutVj = gr.out_degree(Vj,
                                   weight='weight')  # xxxx @directedness 'directed'. Do not delete. Find each edge Vj->Vk that points out of Vj and sum their weights to give WoutVj. Directed graph version.
            ...
            WSVj = scores_array[
                Vj]  # Get the most recent WS score from 'scores_array' for Vj (these are seeded right at the beginning with an arbitrary value)
            # print('This is the most recent WSVj score:')
            # print(WSVj)
            # print('in the calculation of WSVi for this node Vj:')
            # print(Vj)
            score = score + ((gr[Vj][Vi][
                                  'weight'] * WSVj) / WoutVj)  # Do the rhs of the TextRank equation calculation
...
def find_all_gw_scores(gr, scores_array, nf2, dev, d=.85, max_iterations=100, min_delta=0.0000000001):
    ...
    for i in range(max_iterations):  # Only go round this loop max_iterations (100) times for each node.
        ...
        for Vi in nodes:  # For each node Vi in the graph...
            WSVi = find_global_weight_score(Vi, gr, d, min_value, graph_size, scores_array,
                                            i)  # ...find its global weight score, i carried for printing/debugging.
            # if Vi < 100:
            # print('\nNode', Vi)
            ...
            diff += abs(scores_array[
                            Vi] - WSVi)  # Increment 'diff' with the difference between this score and the score calculated in the last iteration.
    ...
    while 1:
        if counter <= len(
            ...
            temp1 = (temp[counter][1][3], temp[counter][0], temp[counter][1][1], temp[counter][1][2], temp[counter][1][
                0])  # xxxx Switch to the above line if you want to round off the scores. Be careful because very large texts get much lower scores, and rounding can render all scores equal.
            mylist.append(temp1)
            counter += 1
        else:
            break
...
# Called by find_sentence_graph_scores in this file.
def sort_ranked_sentences(mylist):
    mylist = [item for item in mylist if item[2] == '#+s#' or item[2] == '#+s:i#' or item[2] == '#+s:c#' or item[
        2] == '#+s:p#']  # or item[2] == '#dummy#' # xxxx 'dummy' and '+s:p' included for RANLP. # or item[2] == '#-s:h#']  # or item[2] == '#-s:t#'or item[2] == '#-s:q#']
...
def make_sentence_graph_nodes(myarray, text, section_labels, parasenttok):
    fill_sentence_array(myarray, text,
                        section_labels)  # Fill array 'myarray' with the fully processed version of the text, one sentence per entry in the array. Each sentence is thus associated with an array key number which represents its position in the text.
    add_to_sentence_array(myarray,
                          parasenttok)  # Add each original unprocessed sentence to 'myarray' at the appropriate key point, so they can be returned in the user feedback later. 'Unprocessed' means following tidy-up, after sentence tokenisation, but before word tokenisation and all the other pre-processing
...
def sample_nodes_for_figure(graph, nodes, cat):
    ...
    list.reverse(
        temp0)  # Re-sort from greatest to smallest so that the sample graph contains the node with the most successors
