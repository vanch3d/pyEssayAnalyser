...
# Called by debora_results_se in this file.
def write_to_summary_file(essay_fname, paras, number_of_words, countTrueSent, countTrueSentChars, countFalseSent,
                          countSentLen, countAvSentLen,
                          countAssQSent, countTitleSent, countTableEnt, countListItem, nodes, edges, edges_over_sents,
                          len_headings, ranked_global_weights, b_last, countProseSents, countProseChars, len_refs,
                          refsheaded, late_wc,
                          appendixheaded, introheaded, i_first, i_last, i_toprank, countIntroSent, countIntroChars,
                          percent_body_i,
                          conclheaded, c_first, c_last, c_toprank,
                          countConclSent, countConclChars, percent_body_c, nf, nf2):
    print(' ')
    # nf2.write('\n')
    ...
    nf2.write('; ')
    nf2.write(
        'heads; ')  # Total number of true headings. Currently '#-s:h#', '#-s:H#','#-s:t#','#-s:s#','#-s:d#','#-s:l#'
...
def get_essay_stats_se(gr_se, text, headings, ranked_global_weights, reorganised_array):
    ...
    for item in reorganised_array:  # For each member of the list/array (each sentence)
        ...
        if label == '#+s#' or label == '#+s:i#' or label == '#+s:c#' or label == '#+s:s#' or label == '#+s:p#':
            ...
            truesents.append(
                item[1])  # Get a list of the node numbers for the true sentences to use later in making a sample graph
        else:
...
def print_processing_times(getassdatatime, processasstexttime, processtbindextime, startassdatatime, startfiletime,
                           texttime, structime, se_scorestime, ke_scorestime, nf2):
...
def debora_write_results_se(essay_fname, paras, rankorder, number_of_words, countTrueSent, countTrueSentChars,
                            countFalseSent, countSentLen, countAvSentLen, nodes, edges, edges_over_sents,
                            ranked_global_weights, reorganised_array, section_names, section_labels, headings,
                            len_headings,
                            countAssQSent, countTitleSent,
                            countTableEnt, countListItem, b_last, countProseSents, countProseChars, len_refs,
                            refsheaded, late_wc, appendixheaded,
    ...
    write_to_details_file(essay_fname, paras, rankorder, number_of_words,
                          ...
                          ranked_global_weights, reorganised_array, i_toprank, c_toprank,
                          nf, nf2)
    write_to_summary_file(essay_fname, paras, number_of_words, countTrueSent, countTrueSentChars, countFalseSent, countSentLen, countAvSentLen,
                          countAssQSent, countTitleSent, countTableEnt, countListItem, nodes, edges, edges_over_sents,
                          len_headings, ranked_global_weights, b_last, countProseSents, countProseChars, len_refs, refsheaded, late_wc,
                          appendixheaded, introheaded, i_first, i_last, i_toprank, countIntroSent, countIntroChars, percent_body_i,
                          conclheaded, c_first, c_last, c_toprank, countConclSent, countConclChars, percent_body_c, nf, nf2)
