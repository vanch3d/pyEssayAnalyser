...
for dirname in tempfilelist:
    ...
    if dirname.startswith(
            'AAA_SentExtrac_Results_'):  # Only delete the temp dirs that you have created in an earlier run of this program.
        shutil.rmtree(fullpath)  # Do the deletion

tempdir1 = tempfile.mkdtemp(suffix='', prefix='AAA_SentExtrac_Results_',
                            dir=None)  # Create a new directory for the essay results files to go in. Make it a temporary directory. Temporary files are stored here: c:\\users\\debora\\appdata\\local\\temp
cwdir = os.getcwd()  # Get current working directory
filelist = os.listdir(
    cwdir)  # Set var 'filelist' to a list of files in cwdir. 'listdir' reads the dir you've given it, and returns a list of files.
newfilename2 = os.path.join(tempdir1, 'sent_extract_results_summary.txt')
nf2 = codecs.open(newfilename2, 'w',
                  encoding='utf-8')  # Open 'newfilename2' (for writing to) and set open file to var 'nf2'
...
for essay_fname in filelist:  # For each file in the current directory...
    ...
    if essay_fname[-3:] == 'txt':  # If a file name ends in 'txt'...
        ...
        nf = codecs.open(newfilename, 'w',
                         encoding='utf-8')  # Open 'newfilename' (for writing to) and set open file to var 'nf'
